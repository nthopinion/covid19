...
class PostDao {
  ...
  async changeQnAcontainer(language) {
    if (language === "English") {
      ...
      this.collections.questionContainerId =
        this.containerIds.questionContainerId + "_" + language;
      this.collections.answerContainerId =
        this.containerIds.answerContainerId + "_" + language;
    }
    debug("Setting up the container...");
    const qResponse = await this.database.containers.createIfNotExists({
      id: this.collections.questionContainerId,
  ...
  async updateQuestion(item, containerName) {
    ...
    const { resource: replaced } = await container.item(item.id).replace(doc);
    return replaced;
  }
  ...
  async updateLike(itemId, containerName) {
    ...
    const { resource: replaced } = await container.item(itemId).replace(doc);
    return replaced;
  }
